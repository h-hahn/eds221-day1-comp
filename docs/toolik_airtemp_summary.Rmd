...
```{r}
toolik_weather <- readr::read_csv(here::here("data", "data-raw", "toolik_weather.csv"), na = "#N/A") |> # pipe operator
  clean_names()
```
...
myAwesomeChart <- ggplot(data = toolik_weather, aes(x = month, y = daily_air_temp_mean_c)) +
  ...
  scale_color_gradient(low = "cornflowerblue", high = "palegreen3") +
  theme_light() +
  labs(title = "Toolik Station daily air temperature",
       x = "Month",
       y = "Daily mean air temperatures (Celsius)")

# ggsave(here::here("figs", "myAwesomeChart.png"), plot = myAwesomeChart, height = 5, width = 6)
```
